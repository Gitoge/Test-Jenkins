...
public List<PostesReferenceActesDTO> findByCodePostes(String code) {
    ...
    return postesReferenceActesRepository.findByCodePostes(code).stream().map(postesReferenceActesMapper::toDto).collect(Collectors.toCollection(LinkedList::new));
}
}
