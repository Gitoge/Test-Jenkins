    ...
    public ResponseEntity<Roles> createRoles(@Valid @RequestBody Roles roles) throws URISyntaxException {
        ...
        if (roles.getId() != null) {
         ...
         if (verifCodeRoles.isPresent()) {
             throw new BadRequestAlertException("Le code '"+roles.getCode()+"' est déjà utilisé : '"+verifCodeRoles.get().getCode()+"'", ENTITY_NAME, "Le code '"+verifCodeRoles.get().getCode()+"' est déjà utilisé par le role  : '"+verifCodeRoles.get().getLibelle()+"'.");
         }
        }
        ...
        if (roles.getLibelle() == null || roles.getLibelle().trim().equals("")) {
         ...
         if (verifLibelleRoles.isPresent()) {
             throw new BadRequestAlertException("Le Libelle '"+verifLibelleRoles.get().getLibelle()+"' est déjà utilisé  par un Role '", ENTITY_NAME, "Le libelle '"+verifLibelleRoles.get().getLibelle()+"' est déjà utilisé . '");
         }
        }
