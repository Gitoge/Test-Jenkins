...
public class ActionsResource {
        ...
        if (actions.getId() != null) {
            ...
            if (actions.getCode() == null ||  actions.getCode().trim().equals("")) {
             ...
             if (verifCodePages.isPresent()) {
                 throw new BadRequestAlertException("Le code '"+actions.getCode()+"' est déjà utilisé : '"+verifCodePages.get().getCode()+"'", ENTITY_NAME, "Le code '"+verifCodePages.get().getCode()+"' est déjà utilisé'.");
             }
            }
    ...
    public ResponseEntity<Actions> updateActions(@PathVariable(value = "id", required = false) final Long id, @Valid @RequestBody Actions actions)
