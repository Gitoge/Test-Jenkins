...
public class DirectionResource {
    ...
    public DirectionResource(DirectionRepository directionRepository, DirectionServiceImpl directionServiceImpl,DirectionService directionService) {
    ...
    public ResponseEntity<Direction> createDirection(@Valid @RequestBody Direction direction) throws URISyntaxException {
        ...
        if (direction.getCode() == null ||  direction.getCode().trim().equals("")) {
         ...
         if (verifCodeDirection.isPresent()) {
             throw new BadRequestAlertException("Le code '"+direction.getCode()+"' est déjà utilisé : '"+verifCodeDirection.get().getCode()+"'", ENTITY_NAME, "Le code '"+verifCodeDirection.get().getCode()+"' est déjà utilisé  : '.");
         }
        }
        ...
        if (direction.getLibelle() == null || direction.getLibelle().trim().equals("")) {
         ...
         if (verifLibelleDirection.isPresent()) {
             throw new BadRequestAlertException("Le Libelle '"+verifLibelleDirection.get().getLibelle()+"' est déjà utilisé'", ENTITY_NAME, "Le libelle '"+verifLibelleDirection.get().getLibelle()+"' est déjà utilisé . '");
         }
        }
