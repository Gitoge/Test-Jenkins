...
public class TypeLocaliteResource {
    ...
    public TypeLocaliteResource(TypeLocaliteRepository typeLocaliteRepository, TypeLocaliteServiceImpl typeLocaliteServiceImpl) {
    ...
    public ResponseEntity<TypeLocalite> createTypeLocalite(@Valid @RequestBody TypeLocalite typeLocalite) throws URISyntaxException {
        ...
        if (typeLocalite.getCode() == null ||  typeLocalite.getCode().trim().equals("")) {
         ...
         if (verifCodeTypeLocalite.isPresent()) {
             throw new BadRequestAlertException("Le code '"+typeLocalite.getCode()+"' est déjà utilisé : '"+verifCodeTypeLocalite.get().getCode()+"'", ENTITY_NAME, "Le code '"+verifCodeTypeLocalite.get().getCode()+"' est déjà utilisé  : '.");
         }
        }
        ...
        if (typeLocalite.getLibelle() == null || typeLocalite.getLibelle().trim().equals("")) {
         ...
         if (verifLibelleTyepLocalite.isPresent()) {
             throw new BadRequestAlertException("Le Libelle '"+verifLibelleTyepLocalite.get().getLibelle()+"' est déjà utilisé'", ENTITY_NAME, "Le libelle '"+verifLibelleTyepLocalite.get().getLibelle()+"' est déjà utilisé . '");
         }
        }
    ...
    public ResponseEntity<List<TypeLocalite>> getAllTypeLocalites(@org.springdoc.api.annotations.ParameterObject Pageable pageable) {
        ...
        Page<TypeLocalite> page = typeLocaliteRepository.findAll(pageable);
